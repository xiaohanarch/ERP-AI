...
| 02 | ERP AI 技术架构 L0：七层结构与两条贯穿线 | L0 接入域 `legacy-erp-ap/.../web/`（ERP 页内嵌 copilot）+ `workbuddy/`（助手平台）+ `scripts/headless_mcp.py`（无头）+ `erp-ai-hub/hub/events.py`（事件触发）；L1 网关 `erp-ai-action/`；L2 Hub `erp-ai-hub/`；L3 语义 `erp-ai-context/`；L4 存量 `legacy-erp-ap/`；模型网关 `erp-ai-action/app/modelgw/`。两条贯穿线：身份（T1→T2→T3，见 [token-design.md](token-design.md)）与评测（`eval/`） | `python scripts/selfcheck.py`（四问） |
...
| 04 | 租户体系下的 harness | 标品资产层 `harness-assets/standard/{skills,rules,guardrails,prompts,evalsets}` + 租户层 `tenant-east/`、`tenant-uni/`；三层解析器 `erp-ai-hub/hub/assets/resolver.py`（Standard→Partner→Tenant，仅 `overridable` 可覆盖，护栏只加严）；租户语义叠加 `erp-ai-context/overlays/` | `python scripts/tenant_checks.py`（六项）；WorkBuddy「解析查看器」页 |
...
| 13 | 工程之外还有五个问题 | （组织/合规/数据治理等，非代码章）演示叙事落位：六幕剧本每幕的「讲解词」 | [demo-script.md](demo-script.md) |
