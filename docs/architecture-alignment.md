...
| 4 | Ontology 是统一语义模型 + 逻辑核心 | ⚠️ 分阶段（**阶段二切片已落地**） | 统一语义与校验核心（已达成）+ 本体驱动结构第一切片（增量段派生字段 → 构建期生成 → BO API `ap.invoice.getDerivedField`，Java 零字段特定逻辑），scene_11；表单/权限生成仍为阶段三愿景 |
| 5 | Metadata 是 Ontology 的输入/来源 | ✅ 一致且超预期（**自动喂养已落地**） | 三段式 + 漂移检测做成可检验不变式；投影段实体清单/标签/枚举由 `/metadata` 实时生成（`effective_projection`），人工只维护口径层，scene_11 |
...
| 13 | 标品 core + Metadata/Ontology/配置适配 | ✅ 一致（**P3 已补配置界面**） | 三层 harness：Standard → Partner（制造业行业包）→ Tenant；租户/行业配置**管理端 UI 改完即生效**（DB 层优先，文件层为出厂默认，change log + 审计），scene_10 |
| 14 | SaaS 标品层/租户层隔离，API/Message 扩展 | ✅ 一致（**P2 已补齐 Message 半边**） | 跨租隔离 5/5、成本双租对账、三层解析隔离；BO API 扩展（规格即工具）+ 消息订阅（topic×webhook×HMAC 按租户隔离投递，吊销即停，scene_9） |
...
4. ~~Message 扩展通道~~ —— 租户消息订阅（topic×webhook×HMAC）按租户隔离
   投递出平台，吊销即停（scene_9）；
5. ~~产品化配置界面~~ —— 租户/行业配置从 YAML 文件级升为管理端 UI
   （DB 层优先 + 出厂默认回退 + change log），改完即生效（scene_10）；
6. ~~元数据自动喂养~~ —— 投影段由存量元数据实时生成，人工只维护口径层；
   新增两条漂移不变式（术语目标 / 派生基字段），scene_11；
7. ~~本体驱动结构（阶段二切片）~~ —— 增量段派生字段经构建期生成成为
   BO API（Java 通用求值引擎），scene_11。

**余项**：
1. **多异构系统对接**（文章选型第四条）：MCP 代理只指向一套 Java。哪怕再挂一个只读 OA/CRM
   工具，即可演示「底座不绑定单一 ERP」。
2. **Ontology 驱动的更深层结构**（观点 4 阶段三）：派生字段 API 已落地；表单、权限、
   流程的生成仍为远期。
3. **更多领域与协同深度**（观点 6 的延伸）：当前为 AP × 采购两域一场景；多域编排
   （三方以上、并行委派、agent 间协商）未涉及——当前阶段以「定义好的流程为主」
   （观点 8），协同走显式委派链而非自由协商，是刻意约束。
4. **配置界面的指标口径编辑**：管理端当前支持行业/阈值/术语，指标口径（net_payable
   公式）为只读——口径属于语义资产，放开编辑需配套校验，留作后续。
...
### Message 扩展通道（观点 14 的后半句，P2）

**场景（业务驱动）**：华东制造想让自己租户侧的合规监控实时响应「发票被阻断」——
SaaS 平台不开放核心改造，租户通过**消息订阅**扩展自己的系统。与形态④
（事件触发平台内 Agent）互补且同源：同一 outbox 事件，「事件进 Agent」与
「事件出平台」不是两套事件。

| 位置 | 内容 |
|---|---|
| `erp-ai-action/app/subscriptions/` | 订阅注册/清册/吊销（`POST/GET /gw/subscriptions`，admin/内部密钥）+ 事件扇出（`POST /internal/events/dispatch`，hub 调用）：按（租户 × topic）匹配活跃订阅，逐个投递并审计（event.dispatch） |
| 投递安全 | 每次投递携带 `X-ERP-Signature: sha256=HMAC(密钥, 载荷)` + `X-ERP-Delivery-Id`——租户侧验签防伪造 |
| `legacy-erp-ap/.../OutboxService.republish` | 事件重投（消息平台标准补投能力）：注入 redelivered 标记走首发同链路，保证场景可重跑 |
| `erp-ai-hub/hub/events.py` `gw.py` | 事件线程处理完（形态④归因 + 通知）后调网关分发——双路消费同源 |

**验证**：scene_9（本机起两个 webhook 接收器扮演双租）—— T-EAST 收到投递且载荷/
签名正确、投递审计留痕、T-UNI 订阅收不到 T-EAST 事件（隔离）、EVENT_DIAG 通知与
webhook 同事件双路、吊销即停投。

### 产品化配置界面（观点 13 的配置半边，P3）

**场景（业务驱动）**：实施顾问在管理端把 T-EAST 大额风险阈值 50 万调成 20 万——
不重启、不改文件、不重建镜像，下一问筛查立即按新口径命中；再把 T-UNI 行业从贸易
切到制造业，行业包（术语 + 护栏）立即对 T-UNI 生效。配置语义与 SaaS 产品一致。

| 位置 | 内容 |
|---|---|
| `erp-ai-context/service/config_store.py` | 配置存储：`sem_tenant_config`（DB 层，首次写入播种自文件，version 递增）+ `sem_config_change_log`；未配置 DB/不可达时优雅回落文件层（服务只读照常） |
| `erp-ai-context/service/loader.py` | `load_overlay`：DB 层优先，文件层为出厂默认/回退 |
| `erp-ai-action/app/config_routes.py` | 管理端代理：`GET/PUT/DELETE /gw/tenants/{id}/config` + changes；鉴权在网关（平台管理员任意租 / 租户配置管理员仅本租，`GW_CONFIG_ADMINS_JSON`）；变更入审计（config.update/reset） |
| `erp-ai-hub/hub/assets/resolver.py` | 行业声明改读语义层配置 API（10s 缓存，文件回退）——管理端切行业，护栏包 ≤10s 生效；同时消除了行业声明在两处的重复 |
| WorkBuddy「租户配置」页 | 管理端 UI：行业/阈值/术语编辑 + 出厂默认对照 + 变更记录；`whoami.configAdmin` 门禁导航 |

**验证**：scene_10 —— whoami 权限判定（wangwu/sunba 是、lisi 否）；出厂默认读取 →
保存即生效（DB 版本号 + 语义工具读新口径 + 端到端筛查按 20 万命中）；行业切换
（术语 partner 层立即 + 护栏 ≤10s）；负向（非管理员/跨租 403）；change log + 审计；
恢复出厂回落文件层（含护栏回落验证，保证不污染租检）。

### 元数据自动喂养 + 本体驱动结构（观点 5 / 观点 4 阶段二，P4）

**场景**：本体与存量元数据是同一条管道的两个方向——先把镜子擦亮（喂养），再让
本体当源（驱动）。

| 位置 | 内容 |
|---|---|
| `erp-ai-context/service/loader.py` `effective_projection` | 投影段 = 生成基础层（实体清单/标签/枚举，实时取 `/metadata`）+ 人工口径层（业务术语/维度）；`/metadata` 不可达回落语义文件冷启动清单 |
| `drift/drift_check.py` | 新增两条不变式：口径层术语指向的实体必须存在于存量元数据（TERM_TARGET_DRIFT）；带 compute 的派生字段基字段必须存在于实体元数据（DERIVED_BASE_DRIFT）——喂养校验驱动；预埋 2 处漂移仍全检出 |
| `scripts/gen_derived_fields.py` | 构建期生成器：本体增量段带 compute 的派生字段 → `derived-fields.json`（含语义版本戳；op 校验/重名校验） |
| `legacy-erp-ap/.../DerivedFieldService` | 通用求值引擎（零字段特定逻辑）：声明式 compute（add/subtract），基字段按 snake_case→camelCase 反射读取；规格新增 BO 操作 `ap.invoice.getDerivedField`（spec 1.2.0） |
| 语义版本 | ap-sem-1.0.0 → 1.1.0（增量段新增派生字段 unpaid_cny） |

**验证**：scene_11 —— 投影段实体集合与 `/metadata` 完全一致（含语义文件从未手写的
PurchaseOrderLine/GoodsReceiptLine）；口径层术语叠加；live 实体名直查；漂移新不变式
计数可见且零新增；`unpaid_cny`（INV-A-001：3400-0）含基字段/公式/语义版本取证；
未定义字段 404；调用落审计；版本链同步（spec 1.2.0 / ap-sem-1.1.0）。

## 六、一句话总结

> 14 条判断与本仓的关系：**13 条已实现且已验证（除 4 外全部，含 API 与 Message
> 双扩展通道、多领域委派协同、元数据自动喂养与配置管理端）；观点 4 分阶段推进——
> 统一语义与校验核心已达成，本体驱动结构的第一切片（派生字段 → 生成 → BO API）
> 已落地，表单/权限生成留作阶段三**。冲突不在代码里，在表述里：把「底座式内生」
> 说清（A）、把「本体核心」分阶段（B）、把「替换的对冲」显式化（C）——这 14 条即构成
> 一个自洽、且大部分已被本仓证实而非证伪的架构纲领。
